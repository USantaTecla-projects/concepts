...
export class CorredtedExamItemDatasource extends InfiniteScrollDatasource<Exam> {
  constructor(private examService: ExamService, private authStore: AuthStore) {
    super();
  }

  fetchData(pageNumber: number): Observable<Page<Exam>> {
    return this.examService.getUserExams(this.authStore.user$, pageNumber, false);
  }
}
